...
namespace KoiTradding.BLL.Services
{
    public class OrderDetailService
    {
        ...
        public async Task<bool> UpdateOrderDetailAsync(OrderDetail orderDetail)
        {
            ...
            // Business-specific validations
            ValidateOrderDetail(orderDetail, isUpdate: true);
        ...
        private void ValidateOrderDetail(OrderDetail orderDetail, bool isUpdate = false)
        {
            if (!isUpdate && orderDetail.OrderDetailId != 0)
                throw new ArgumentException("OrderDetail ID should not be set for a new order detail.", nameof(orderDetail.OrderDetailId));

            if (orderDetail.OrderId == null || orderDetail.OrderId <= 0)
                throw new ArgumentException("Order ID must be provided and greater than zero.", nameof(orderDetail.OrderId));
            ...
            if (orderDetail.Quantity == null || orderDetail.Quantity <= 0)
                throw new ArgumentException("Quantity must be provided and greater than zero.", nameof(orderDetail.Quantity));
